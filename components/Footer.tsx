import Link from "next/link";
import FooterClock from "./FooterClock";
import BackToTopButton from "./BackToTopButton";

const NAV_LINKS = [
  { href: "/projects", label: "Projets" },
  { href: "/experience", label: "Expérience" },
  { href: "/community", label: "Communauté" },
  { href: "/notes", label: "Notes" },
  { href: "/contact", label: "Contact" },
  { href: "/cv", label: "CV en ligne" },
  { href: "/branding", label: "Branding" },
];

const SOCIALS = [
  {
    name: "GitHub",
    href: "https://github.com/vincenthirtz",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="size-4" aria-hidden="true">
        <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
      </svg>
    ),
  },
  {
    name: "LinkedIn",
    href: "https://linkedin.com/in/hirtzvincent",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="size-4" aria-hidden="true">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
  },
  {
    name: "RSS",
    href: "/feed.xml",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="size-4" aria-hidden="true">
        <path d="M6.18 15.64a2.18 2.18 0 012.18 2.18C8.36 19.01 7.37 20 6.18 20 5 20 4 19.01 4 17.82a2.18 2.18 0 012.18-2.18zM4 4.44A15.56 15.56 0 0119.56 20h-2.83A12.73 12.73 0 004 7.27V4.44zm0 5.66a9.9 9.9 0 019.9 9.9h-2.83A7.07 7.07 0 004 12.93v-2.83z" />
      </svg>
    ),
  },
];

export default function Footer() {
  return (
    <footer className="relative border-t px-6 py-12" style={{ borderColor: "var(--border)" }}>
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col gap-10 md:flex-row md:items-start md:justify-between">
          {/* Gauche : logo + tagline + disponibilité */}
          <div>
            <Link href="/" className="font-mono text-xl font-bold">
              VH<span className="text-[var(--color-accent)]">.</span>
            </Link>
            <p className="mt-3 max-w-xs text-sm text-[var(--fg-muted)]">
              Vincent Hirtz — Lead Developer Front-End, basé à Lyon, disponible en remote ou sur
              site.
            </p>

            {/* Badge disponibilité */}
            <div className="mt-3 inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1">
              <span className="relative flex size-2">
                <span className="absolute inline-flex size-full animate-ping rounded-full bg-emerald-400 opacity-75" />
                <span className="relative inline-flex size-2 rounded-full bg-emerald-500" />
              </span>
              <span className="text-xs font-medium text-emerald-400">Disponible pour missions</span>
            </div>

            <div className="mt-4 flex items-center gap-2">
              <span className="font-mono text-[10px] uppercase tracking-widest text-[var(--fg-dim)]">
                Lyon, France
              </span>
              <FooterClock />
            </div>
          </div>

          {/* Centre : nav */}
          <nav aria-label="Navigation du pied de page">
            <ul className="grid grid-cols-2 gap-x-12 gap-y-2 md:grid-cols-1">
              {NAV_LINKS.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-[var(--fg-muted)] transition-colors hover:text-[var(--color-accent)]"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </nav>

          {/* Droite : réseaux + retour en haut */}
          <div className="flex flex-col items-start gap-4 md:items-end">
            {/* Liens sociaux */}
            <div className="flex items-center gap-3">
              {SOCIALS.map((social) => (
                <a
                  key={social.name}
                  href={social.href}
                  target={social.href.startsWith("/") ? undefined : "_blank"}
                  rel={social.href.startsWith("/") ? undefined : "noopener noreferrer"}
                  aria-label={social.name}
                  className="flex size-9 items-center justify-center rounded-full border text-[var(--fg-muted)] transition-colors hover:border-[var(--color-accent)] hover:text-[var(--color-accent)]"
                  style={{ borderColor: "var(--border-strong)" }}
                >
                  {social.icon}
                </a>
              ))}
            </div>

            <BackToTopButton />

            <div
              className="group cursor-default font-mono text-[10px] uppercase tracking-widest text-[var(--fg-dim)]"
              title="Konami Code"
            >
              <span className="opacity-0 transition-opacity group-hover:opacity-100">
                Tip : essayez &uarr;&uarr;&darr;&darr;&larr;&rarr;&larr;&rarr; B A
              </span>
            </div>
          </div>
        </div>

        {/* Barre de bas de page */}
        <div
          className="mt-12 flex flex-col items-start justify-between gap-3 border-t pt-6 text-xs text-[var(--fg-dim)] md:flex-row md:items-center"
          style={{ borderColor: "var(--border)" }}
        >
          <div>&copy; 2026 Vincent Hirtz. Tous droits r&eacute;serv&eacute;s.</div>
          <div className="flex items-center gap-4">
            <Link
              href="/mentions-legales"
              className="transition-colors hover:text-[var(--color-accent)]"
            >
              Mentions l&eacute;gales
            </Link>
            <a href="/sitemap.xml" className="transition-colors hover:text-[var(--color-accent)]">
              Plan du site
            </a>
          </div>
          <div>
            Fait avec <span className="text-[var(--color-accent)]">&hearts;</span>
            {" et beaucoup de café."}
          </div>
        </div>
      </div>
    </footer>
  );
}
